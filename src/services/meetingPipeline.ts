...
export interface MeetingPipelineResult {
  ...
  reason?: string;
  fallbackDraftCreated?: boolean; // Track if we created a regular draft as fallback
}
...
export class MeetingPipelineService {
  ...
  constructor(responseService?: ResponseService) {
    ...
    // Accept ResponseService for fallback to regular auto-draft generation
    this.responseService = responseService || null;
    this.autoGeneratedDraftModel = new AutoGeneratedDraftModel();
    // NOTE: MeetingResponseGeneratorService is now created per-request via ServiceFactory
    // since it requires user-specific providers (ICalendarProvider, IEmailProvider)
  }
  ...
  async processEmailForMeetings(
    ...
    responseGenerator?: MeetingResponseGeneratorService // ✅ COMPOSIO MIGRATION: Pass per-request instance from ServiceFactory
    ...
    try {
      ...
      if (email.from && email.from.includes('parthahir012001@gmail.com')) {
        ...
        await this.storePipelineResult(result, dbId);
        logger.debug({ emailId: email.id, subject: email.subject }, 'meeting.email.loop_prevention');
        return result;
      }
      */
      ...
      // Fetch user's timezone before meeting detection (CRITICAL FIX!)
      let userTimezone: string | null = null;
      try {
        userTimezone = await TimezoneService.getUserTimezoneFromDB(userId);
        if (userTimezone) {
                  } else {
                  }
      } catch (error) {
      ...
      // Run meeting detection with user's timezone
      const meetingRequest = await this.meetingDetection.detectMeetingRequest(email, userTimezone);
      ...
      await this.executeInTransaction(async (client) => {
        ...
        if (meetingRequest) {
          ...
          // ✅ COMPOSIO MIGRATION: Generate intelligent response using provider-based service
          try {
            if (responseGenerator) {
              const response = await responseGenerator.generateMeetingResponse(email, meetingRequestWithId, userId, testMode);
              ...
              logger.debug({ userId: sanitizeUserId(userId) }, 'meeting.response.generator.not.provided');
            }
          } catch (responseError) {
          ...
          // 🆕 OPTION B FALLBACK: Generate regular auto-draft for non-meeting-request emails
          // This ensures personal emails that mention meetings (confirmations, recaps, etc.) still get responses
                    
          try {
            if (!this.responseService) {
              logger.debug({}, 'meeting.fallback.service.unavailable');
            } else {
              // Generate regular auto-draft (bypassing meeting detection entirely)
              const responseRequest = {
                emailId: dbId.toString(),
              ...
              const draftId = `autodraft_fallback_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;

              const autoDraftData = {
                draft_id: draftId,
                original_email_id: dbId,
                subject: smartResponse.subject,
                body: smartResponse.body,
                tone: smartResponse.tone,
                urgency_level: smartResponse.urgencyLevel,
                context_used: {
                  source: 'meeting_pipeline_fallback',
                  route: 'autodraft',
                  reason: 'Meeting-related email but not a meeting request',
                  originalEmail: {
                  ...
                  responseGeneration: {
                    ...
                    reasoning: 'Auto-draft generated as fallback from meeting pipeline'
                  }
                },
              ...
              await this.autoGeneratedDraftModel.createDraftForUser(autoDraftData);

              result.fallbackDraftCreated = true;
                          }
          } catch (fallbackError) {
            ...
            // Don't throw - this is a nice-to-have fallback, not critical
          }
        }
  ...
  async healthCheck(): Promise<{ 
    ...
    try {
      ...
      // TODO: Response generator health check disabled until Phase 6F migration
      // const responseHealth = await this.responseGenerator.healthCheck();
      const responseHealth = { status: 'migrating', isHealthy: false, message: 'Migration in progress' };
