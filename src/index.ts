...
async function initializeServices() {
  ...
  // responseService = new ResponseService(aiService, contextService, gmailService); // ❌ REMOVED: Now uses ServiceFactory with emailProvider + userId
  learningService = new LearningService(aiService);
  calendarService = new CalendarService();
  calendarModel = new CalendarModel();
  meetingDetectionService = new MeetingDetectionService();
  // autoSchedulingService = new AutoSchedulingService(); // ❌ REMOVED: Now requires ICalendarProvider + userId, use ServiceFactory
  meetingPipelineService = new MeetingPipelineService(undefined); // Pass undefined, will be created per-user via ServiceFactory
  // meetingConfirmationService = new MeetingConfirmationService(); // ❌ REMOVED: Now requires ICalendarProvider + userId, use ServiceFactory
  ...
  intelligentEmailRouter = new IntelligentEmailRouter(undefined); // Pass undefined, will be created per-user via ServiceFactory
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    // ✅ COMPOSIO MIGRATION: Use OAuth2 for token exchange (not Gmail API)
    // Exchange authorization code for tokens
    const tokens = await gmailService.setTokens(code as string);
    console.log('✅ OAuth token exchange successful!');

    // Get user email from OAuth2 userinfo (not Gmail API)
    try {
      ...
      if (gmailAddress) {
        ...
        // ✅ COMPOSIO MIGRATION: Create user profile without saving Google tokens
        // User will connect Gmail/Calendar via Composio after OAuth
        const userId = await tokenStorageService.createUserWithPendingComposio(gmailAddress);

        console.log(`💾 User profile created: ${userId} (auth_method=pending_composio)`);
        console.log(`📧 Gmail address: ${gmailAddress}`);
        console.log(`⏭️  User needs to connect via Composio for Gmail/Calendar access`);
        ...
        throw new Error('Missing Gmail address from OAuth');
      }
    } catch (error) {
...
app.put('/api/user/scheduling-link', authMiddleware.authenticate, async (req, res) => {
  try {
    console.log('🔗 PUT /api/user/scheduling-link called');
    const userId = getUserId(req);
    const { schedulingLink } = req.body;
    console.log(`🔗 User ${userId.substring(0, 8)}: Updating scheduling link to: ${schedulingLink}`);

    if (!schedulingLink || typeof schedulingLink !== 'string') {
      console.log('🔗 ❌ Invalid scheduling link provided');
    ...
    if (success) {
      console.log(`✅ Updated scheduling link for user ${userId.substring(0, 8)}: ${validationResult.platform}`);
      res.json({
        success: true,
...
app.delete('/api/user/scheduling-link', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    if (success) {
      console.log(`✅ Removed scheduling link for user ${userId.substring(0, 8)}`);
      res.json({
        success: true,
...
app.get('/emails/fetch', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    // ✅ COMPOSIO MIGRATION: Use EmailProvider abstraction
    ...
    for (const email of emails) {
      try {
        // Convert provider EmailMessage to Gmail-compatible format for parseEmail
        // Type cast is safe because provider returns Gmail-compatible data structure
        const parsedEmail = gmail.parseEmail(email as any);
...
app.post('/auto-drafts/:id/send', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    // Send email via provider with threading support
    ...
    if (draftWithEmail.relationship_type === 'meeting_response' && draftWithEmail.context_used) {
      try {
        ...
        console.log(`📅 Processing meeting response (${actionTaken}) for draft ${draftId}...`);
        ...
        if (actionTaken === 'accepted' && context.meetingRequest?.preferredDates?.[0]) {
          ...
          console.log(`📅 Creating calendar event for accepted meeting: ${meetingTitle}`);
          console.log(`🕐 Time: ${proposedTime}`);
          console.log(`👤 With: ${senderEmail}`);

          // Create calendar event directly using calendar service
          try {
            // ✅ COMPOSIO MIGRATION: Use CalendarProvider abstraction
            ...
            calendarBooked = true;
            console.log(`✅ Calendar event created via ${calendarProvider.getProviderName()}: ${calendarEventId}`);

          } catch (calendarError) {
            console.warn('⚠️ Failed to create calendar event (continuing with email send):', calendarError);
            // Don't fail the entire operation if calendar booking fails
          }
        } else {
          console.log(`📝 Meeting response type: ${actionTaken} - no calendar event needed`);
        }

      } catch (meetingError) {
        console.warn('⚠️ Failed to process meeting response (continuing with email send):', meetingError);
      }
    }
    ...
    if (draftWithEmail.context_used && draftWithEmail.context_used.meetingRequest && draftWithEmail.context_used.meetingRequest.id) {
      try {
        ...
        console.log(`📅 [LEGACY] Creating meeting confirmation for draft ${draftId}...`);

        // ✅ COMPOSIO MIGRATION: Use ServiceFactory to get meetingConfirmationService
        const { ServiceFactory } = await import('./utils/serviceFactory');
        const services = ServiceFactory.createForUser(userId);
        const meetingConfirmationService = await services.getMeetingConfirmationService();
        ...
        console.log(`✅ [LEGACY] Meeting confirmation created: ${meetingConfirmationId}`);

      } catch (confirmationError) {
        console.warn('⚠️ Failed to create legacy meeting confirmation:', confirmationError);
      }
    }

    console.log(`✅ Draft ${draftId} sent successfully via Gmail API`);
    console.log(`📧 Gmail Message ID: ${sendResult.id}`);
...
app.post('/auto-drafts/:id/decline', authMiddleware.authenticate, async (req, res) => {
  try {
    ...
    console.log(`🚫 [DECLINE] User ${userId} declining draft ${draftId}: "${reason.substring(0, 50)}..."`);
    ...
    console.log(`📧 [DECLINE] Original meeting from: ${context.originalEmail.from}`);
    console.log(`📅 [DECLINE] Meeting type: ${context.meetingRequest.type}`);
    ...
    console.log(`🤖 [DECLINE] Generating AI decline response...`);
...
${closing}${name ? '\n' + name : ''}`;
    ...
    console.log(`✅ [DECLINE] New decline draft created: ${newDraftId}`);
    ...
    console.error('❌ [DECLINE] Error generating decline response:', error);
    res.status(500).json({
  ...
  if (process.argv[2] === 'test') {
    ...
    app.post('/response/generate-smart', authMiddleware.authenticate, async (req, res) => {
      try {
        ...
        // ✅ MIGRATION: Use ServiceFactory to get ResponseService with providers
        const { ServiceFactory } = await import('./utils/serviceFactory');
        const services = ServiceFactory.createFromRequest(req);
    ...
    app.post('/auto-scheduling/process-meeting', async (req, res) => {
      try {
        ...
        const userId = meetingRequest.user_id;

        // ✅ COMPOSIO MIGRATION: Use ServiceFactory to get autoSchedulingService
        const { ServiceFactory } = await import('./utils/serviceFactory');
        const services = ServiceFactory.createForUser(userId);
        const autoSchedulingService = await services.getAutoSchedulingService();
    ...
    app.post('/auto-scheduling/create-hold', async (req, res) => {
      try {
        ...
        // Get meeting request to extract userId
        const meetingQuery = 'SELECT user_id FROM meeting_requests WHERE id = $1';
        const meetingResult = await pool.query(meetingQuery, [meetingRequestId]);
        ...
        const userId = meetingResult.rows[0].user_id;
    ...
    app.post('/auto-scheduling/cleanup-holds', async (req, res) => {
      try {
        ...
        // ✅ COMPOSIO MIGRATION: Use ServiceFactory (system-wide operation, no specific user)
        // Note: cleanupExpiredHolds() is a database-only operation that doesn't require calendar access
        const { ServiceFactory } = await import('./utils/serviceFactory');
        const services = ServiceFactory.createForUser('system'); // Use dummy userId for system operations
        const autoSchedulingService = await services.getAutoSchedulingService();
    ...
    app.get('/auto-scheduling/health', async (req, res) => {
      try {
        ...
        // ✅ COMPOSIO MIGRATION: Use ServiceFactory (system-wide operation, no specific user)
        // Note: healthCheck() is a database-only operation that doesn't require calendar access
        const { ServiceFactory } = await import('./utils/serviceFactory');
        const services = ServiceFactory.createForUser('system'); // Use dummy userId for system operations
        const autoSchedulingService = await services.getAutoSchedulingService();
...
// Webhook management endpoints (Google Pub/Sub - Legacy)
app.get('/webhook-status', async (_req, res) => {
  try {
    // ⚠️ COMPOSIO MIGRATION: Google webhooks disabled when using Composio triggers
    if (process.env.DISABLE_GOOGLE_WEBHOOKS === 'true') {
      return res.json({
        disabled: true,
        message: 'Google webhooks disabled - using Composio triggers',
        webhooks: [],
        totalActive: 0,
        needingRenewal: 0
      });
...
app.post('/webhook-renewal/manual', async (_req, res) => {
  try {
    ...
    if (process.env.DISABLE_GOOGLE_WEBHOOKS === 'true') {
      return res.status(400).json({
        error: 'Google webhooks disabled',
        message: 'Manual renewal not available when using Composio triggers'
      });
    }
...
initializeServices().then(() => {
    ...
    // 🚀 PHASE 5: Composio Webhook Endpoint
    // Receives Gmail trigger notifications from Composio (no base64 encoding, direct JSON)
    app.post('/webhooks/composio', async (req, res) => {
      try {
        ...
        // Acknowledge receipt immediately (same as Google - must respond quickly)
        res.status(200).json({ success: true });

        // Extract Composio webhook data (NO base64 decoding needed!)
        ...
        if (triggerName !== 'GMAIL_NEW_GMAIL_MESSAGE') {
          pinoLogger.warn({
            triggerName,
            supportedTriggers: ['GMAIL_NEW_GMAIL_MESSAGE']
          }, 'webhook.composio.unknown_trigger');
          return;
        }

        // Validate payload
        if (!payload || !payload.historyId) {
          pinoLogger.error({ payload }, 'webhook.composio.invalid_payload');
          return;
        }
        ...
        pinoLogger.info({
          ...
          userId: sanitizeUserId(connectedAccountId)
        }, 'webhook.composio.notification_parsed');

        // PRODUCTION: Idempotency check to prevent duplicate processing
        // Use historyId as unique identifier for this webhook event
        const lockKey = `webhook:composio:${payload.historyId || Date.now()}`;
        const lockAcquired = await redis.acquireLock(lockKey, 60); // 1 min TTL (only for deduplication)
        ...
        // Release lock immediately after deduplication check
        // Lock is only for preventing duplicate webhook acceptance, not for processing duration
        setTimeout(() => {
          redis.releaseLock(lockKey).catch(err => {
            pinoLogger.debug({
        ...
        // Process the notification for the specific user (connectedAccountId = userId)
        // ✅ COMPOSIO ARCHITECTURE: One trigger per user, so we process for that user only
        (async () => {
          try {
            pinoLogger.debug({
              userId: sanitizeUserId(connectedAccountId),
              historyId: payload.historyId
            }, 'webhook.composio.user.processing_start');

            // Create isolated service container for this user
            const { ServiceFactory } = await import('./utils/serviceFactory');
            const services = ServiceFactory.createForUser(connectedAccountId);
            const emailProvider = await services.getEmailProvider();
            const gmail = await services.getGmailService(); // Still needed for parseEmail helper

            // Process the notification for this specific user with isolated provider
            ...
            pinoLogger.info({
              ...
              userId: sanitizeUserId(connectedAccountId),
              totalProcessed: webhookHeartbeat.totalProcessed
            }, 'webhook.composio.processing_complete');
          } catch (error) {
            pinoLogger.error({
              historyId: payload.historyId,
              userId: sanitizeUserId(connectedAccountId),
              error: error instanceof Error ? error.message : String(error)
            }, 'webhook.composio.processing_failed');
          }
        })();
        ...
        pinoLogger.error({
          error: error instanceof Error ? error.message : String(error),
          body: req.body
        }, 'webhook.composio.handler_error');
        // Response already sent on line above, don't send again
      }
    });
      ...
      try {
        ...
        if (!targetEmail) {
          ...
          const processingPromises = activeUsers.map(async (userTokens) => {
            try {
              ...
              // ✅ COMPOSIO MIGRATION: Create isolated service container for each user
              ...
              // Process the notification for this user with isolated provider
              await processGmailNotificationForUser(notification, userTokens.userId, emailProvider, gmail);
        ...
        const processingPromises = [targetUserTokens].map(async (userTokens) => {
          try {
            ...
            // ✅ COMPOSIO MIGRATION: Create isolated service container for this user
            const { ServiceFactory } = await import('./utils/serviceFactory');
            const services = ServiceFactory.createForUser(userTokens.userId);
            const emailProvider = await services.getEmailProvider();
            const gmail = await services.getGmailService(); // Still needed for parseEmail helper
    ...
    // ✅ COMPOSIO MIGRATION: Now accepts emailProvider for provider abstraction + gmail for parseEmail helper
    async function processGmailNotificationForUser(notification: any, userId: string, emailProvider: any, gmail: any) {
      ...
      try {
        ...
        if (notification.messageId) {
          ...
          // Fetch the specific email using provider (fetch single email with query)
          ...
          // Get recent emails using provider
        ...
        async function processEmailSafe(emailData: any): Promise<{
          ...
          try {
            // Parse email content
            const parsedEmail = gmail.parseEmail(emailData as any);
            pinoLogger.debug({
              subject: parsedEmail.subject,
              from: parsedEmail.from,
              userId: sanitizeUserId(userId)
