...
exports.WebhookTestingSuite = void 0;
const tokenStorage_1 = require("../../src/services/tokenStorage");
const gmail_1 = require("../../src/services/gmail");
const webhookRenewal_1 = require("../../src/services/webhookRenewal");
const connection_1 = require("../../src/database/connection");
class WebhookTestingSuite {
    constructor() {
        this.testResults = [];
        this.tokenStorageService = new tokenStorage_1.TokenStorageService();
        this.webhookRenewalService = new webhookRenewal_1.WebhookRenewalService();
    }
    /**
    ...
    async testDatabaseConnection() {
        await this.runTest('Database Connection', async () => {
            const result = await connection_1.pool.query('SELECT 1 as test');
    ...
    async testTokenEncryption() {
        await this.runTest('Token Encryption/Decryption', async () => {
            ...
            // Cleanup
            await connection_1.pool.query('DELETE FROM user_gmail_tokens WHERE user_id = $1', [savedUserId]);
        });
    }
    // 2. Service Initialization Tests
    async testServiceInitialization() {
        await this.runTest('Service Initialization', async () => {
            const renewalService = new webhookRenewal_1.WebhookRenewalService();
    ...
    async testGmailServiceForUsers() {
        await this.runTest('Gmail Service Multi-User Support', async () => {
            ...
            for (const user of activeUsers.slice(0, 2)) { // Test first 2 users
                try {
                    const gmailService = new gmail_1.GmailService();
    ...
    async testWebhookExpirationTracking() {
        await this.runTest('Webhook Expiration Tracking', async () => {
            ...
            // Cleanup
            await connection_1.pool.query('DELETE FROM user_gmail_tokens WHERE user_id = $1', [testUserId]);
        });
    }
    async testWebhookRenewalLogic() {
    ...
    async testExpiringWebhooksDetection() {
    ...
    // 4. Multi-user Tests
    async testMultiUserWebhookIsolation() {
        await this.runTest('Multi-User Webhook Isolation', async () => {
            ...
            if (activeUsers.length < 2) {
                ...
                // Cleanup
                await connection_1.pool.query('DELETE FROM user_gmail_tokens WHERE user_id IN ($1, $2)', [user1Id, user2Id]);
            }
            console.log(`    ✓ Multi-user isolation verified with ${Math.max(activeUsers.length, 2)} users`);
        });
    }
    async testConcurrentWebhookRenewal() {
    ...
    async testInvalidCredentialHandling() {
    ...
    async testWebhookStatusEndpoint() {
    ...
    async testManualRenewalEndpoint() {
    ...
    async testLargeUserBaseScenario() {
    ...
    async testWebhookExpirationEdgeCases() {
...
exports.WebhookTestingSuite = WebhookTestingSuite;
//# sourceMappingURL=webhookTesting.js.map